...
println("**************MEMBER ONBOARDINGG STARTED**************")
...
println("**************MEMBER ONBOARDINGG PASSED**************")
...
println("**************MEMBER EXISTENCE STARTED**************")
 ...
 println(custloan_id)
 response5 = WS.sendRequest(findTestObject('EKYC Flow/Member Existence Check', [('ex_loan_id') : custloan_id, ('ex_req_id') : exreq_id, ('NRIC'): IDV_Nric]))
...
println(custloan_id)
response1 = WS.sendRequest(findTestObject('EKYC Flow/Customer Risk Rating', [('cust_req_id') : custreq_id, ('cust_loan_id') : custloan_id, ('NRIC'): IDV_Nric]))
...
response2 = WS.sendRequest(findTestObject('EKYC Flow/Industry Risk Rating', [('ind_loan_id') : custloan_id, ('ind_req_id') : indreq_id, ('NRIC'): IDV_Nric]))
...
response3 = WS.sendRequest(findTestObject('EKYC Flow/Company Risk Rating', [('com_loan_id') : custloan_id, ('com_req_id') : comreq_id, ('NRIC'): IDV_Nric]))
...
response6 = WS.sendRequest(findTestObject('EKYC Flow/Fully EKYC', [('ekyc_loan_id') : custloan_id, ('ekyc_req_id') : ekycreq_id, ('NRIC'): IDV_Nric]))
...
response7 = WS.sendRequest(findTestObject('EKYC Flow/Server Bio', [('ser_loan_id') : custloan_id, ('ser_req_id') : serreq_id, ('NRIC'): IDV_Nric]))
...
Loanresponse = WS.sendRequest(findTestObject('EKYC Flow/Loan Onboarding', [('Token') : GlobalVariable.Token, ('loan_id') : custloan_id, ('loan_req_id') : loanreq_id, ('email') : email, ('BRN') : BRN, ('NRIC'): IDV_Nric]))
...
println('**************LOAN ONBOARDING PASSED**************')

'Loan Account Creation'
println('**************LOAN CREATION STARTED**************')
crreq_id = (((Math.floor(Math.random() * 9000000000)) as long) + 1000000000)
println(custloan_id)
CRResponse = WS.sendRequest(findTestObject('EKYC Flow/Loan Acc Creation', [('Token') : GlobalVariable.Token, ('crloan_id') : custloan_id, ('crreq_id') : crreq_id, ('BRN') : BRN, ('NRIC'): IDV_Nric]))
println(CRResponse.getResponseBodyContent())
Assertions.assertThat(CRResponse.getStatusCode()).isEqualTo(200)
println(WS.verifyElementPropertyValue(CRResponse, 'status', 'Processing'))
println(WS.verifyElementPropertyValue(CRResponse, 'message', 'Loan is being processed'))
WS.delay(20)
